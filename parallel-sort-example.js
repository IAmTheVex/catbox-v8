...
const catbox = require('catbox-vm8');
...
function quicksortParallel(array, left, right, callback) {
	...
	(async function() {
		if (right - left > 1024 << 9) {
			...
			// The following array map creates two new `catbox.Isolate` instances with the functions we
			...
			let threadPromises = [ 0, 0 ].map(async function() {
				let isolate = new catbox.Isolate;
				...
				// Here we give the isolate we just spawned access to the entire `catbox-v8` library
				context.global.setIgnored('catbox', catbox);
				// This gets an `catbox.Reference` to the `quicksortParallel` function that lives in the new
				...
				return function(array, left, right) {
					return new Promise(function(resolve) {
						fnReference.apply(undefined, [ array, left, right, new catbox.Reference(resolve) ]);
					});
				};
			});
			...
			let reference = new catbox.ExternalCopy(array);
